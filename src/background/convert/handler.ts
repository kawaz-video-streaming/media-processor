...
import { cleanupWorkspace, convertMediaToDashStream, generateSubtitleTracks, getVideoMetadata, initializeWorkspace, uploadStreamToStorage, writeMediaToDirectory } from "./utils";
...
export const convertMediaHandler = (storageClient: StorageClient, config: ConvertConfig) =>
    async ({ mediaId, mediaStorageBucket, mediaRoutingKey, mediaName }: Convert) => {
        ...
        try {
            ...
            await convertMediaToDashStream([mediaPath, ...subtitlePaths], mpdPath);
