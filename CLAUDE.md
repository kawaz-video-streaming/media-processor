...
7. Patches `output.mpd` with `<AdaptationSet>` entries for each subtitle track, each including a `<Label>` element with a human-readable track title. Subtitle titles are resolved from the FFprobe `title` tag when meaningful, otherwise derived from the language code via `Intl.DisplayNames`; same-language tracks without distinct titles are disambiguated with a counter suffix (e.g. "English (2)"). Publishes `{ mediaId, percentage: 90, status: 'processing' }` before upload. Upload all workspace files to VOD S3 bucket under `<mediaId>/` key prefix via `storageClient.uploadObjects` (bulk upload); upload progress published as 90–100% (throttled).
...
**Key directories:**
- `src/background/convert/` — entire conversion consumer: `handler.ts` orchestrates, `logic.ts` runs the conversion pipeline, `utils.ts` implements each step, `types.ts` has Zod schema + interfaces, `consts.ts` has module-level constants, `errors.ts` has domain errors, `binding.ts` has AMQP queue/exchange/topic constants
